...
template <size_t n>
inline f32x8 load_aligned(__attribute((aligned(32))) float* ptr);

template <>
inline f32x8 load_aligned<8>(__attribute((aligned(32))) float* ptr) {
    return {_mm256_load_ps(ptr)};
}

template <size_t n>
inline f32x8 load_unaligned(float* ptr);

template <>
inline f32x8 load_unaligned<8>(float* ptr) {
