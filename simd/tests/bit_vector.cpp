...
TEST(i32x4, from) {
    ...
    int32_t actual[4];
    simd::i32x4::from(0, 1, 2, 3).store(simd::as_unaligned_view(actual));
...
TEST(i32x8, from) {
    ...
    simd::i32x8::from(0, 1, 2, 3, 4, 5, 6, 7)
            .store(simd::as_unaligned_view(actual));
...
void test_32_load_store_aligned() {
    alignas(32) SourceT expected[] = {0, 1, 2, 3, 4, 5, 6, 7};
    alignas(32) SourceT actual[T::size];

    auto v = T::load(simd::as_aligned_view<32>(expected));
    v.store(simd::as_unaligned_view(actual));
...
void test_32_load_store_unaligned() {
    ...
    alignas(32) SourceT expected[]          = {-1, 0, 1, 2, 3, 4, 5, 6, 7};
    alignas(32) SourceT actual[1 + T::size] = {-1};

    // do the test twice because one of these will have to be unaligned
    {
        auto v = T::load(simd::as_unaligned_view(expected));
        v.store(simd::as_unaligned_view(actual));

        EXPECT_TRUE(std::equal(expected, expected + T::size, actual));
    }
    {
        auto v = T::load(simd::as_unaligned_view(expected + 1));
        v.store(simd::as_unaligned_view(actual + 1));
...
void test_f32_multiply() {
    ...
    auto v = T::load(simd::as_unaligned_view(input));
    (v * v).store(simd::as_unaligned_view(actual));
...
void test_f32_equality() {
    ...
    auto v1 = T::load(simd::as_unaligned_view(a));
    auto v2 = v1;
    auto v3 = T::load(simd::as_unaligned_view(b));
...
void test_32_hadd() {
    ...
    auto v1       = T::load(simd::as_unaligned_view(a));
    auto v2       = T::load(simd::as_unaligned_view(b));
    auto expected = T::load(simd::as_unaligned_view(c));
